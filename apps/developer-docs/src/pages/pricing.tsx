...
export default function PricingPage() {
  ...
  return (
    <div className="min-h-screen bg-black/75 text-white/92 flex flex-col relative overflow-x-hidden">
      ...
      <main className="flex-1 p-6 lg:p-12 pb-20 relative z-10">
        ...
        <nav className="flex justify-between items-center mb-12">
          <a href="/" className="text-lg font-bold text-green-400/90 hover:text-green-400/80 transition-colors">
            P3 Developer
          </a>
          <div className="flex gap-4">
            <a href="/" className="text-white/92 hover:text-green-400/80 transition-colors text-sm">Dashboard</a>
            <a href="/docs.html" className="text-white/92 hover:text-green-400/80 transition-colors text-sm">Docs</a>
          </div>
        </nav>
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {PLANS.map((plan) => (
            <div
              key={plan.id}
              className={`
                relative bg-gray-900/92 md:bg-gray-900/65 backdrop-blur-md p-8 rounded-2xl border border-gray-700/60
                overflow-hidden group hover:border-green-500/30 transition-colors
                ${plan.featured ? 'border-2 border-green-500/50 shadow-lg shadow-green-500/10' : ''}
              `}
            >
              {plan.featured && (
              ...
              <div className="relative">
                ...
                <p className="text-5xl font-bold text-green-400/90 mb-6">{plan.price}</p>
                <ul className="space-y-3 mb-8">
                  {plan.bullets.map((b) => (
                    <li key={b} className="text-white/92 flex items-center gap-2">
                      <span className="w-1.5 h-1.5 rounded-full bg-green-400/90 shrink-0" />
                      {b}
                    </li>
                  ))}
                </ul>
                <button
                  ...
                  disabled={!!loading}
                  className="w-full bg-green-600/75 hover:bg-green-600/80 disabled:opacity-50 text-white font-bold py-4 px-8 rounded-lg transition-colors shadow-[0_0_15px_rgba(34,197,94,0.3)]"
                >
                  {loading === plan.id ? (
        ...
        <div className="fixed bottom-0 left-0 right-0 z-20 flex items-center justify-between gap-4 px-4 py-3 bg-gray-900/92 md:bg-gray-900/65 backdrop-blur-md border-t border-gray-700/60">
