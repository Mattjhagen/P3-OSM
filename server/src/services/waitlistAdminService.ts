...
const applyInviteUpdate = async (ids: string[]) => {
  ...
  let { error } = await supabase.from('waitlist').update(basePayload).in('id', ids);
  if (!error) return;
  ...
  if (fallback.error) {
    throw new WaitlistAdminError(
      500,
      `Failed to update waitlist invite status: ${fallback.error.message}`
    );
  }
};

const resolveCountForStatus = async (status?: 'PENDING' | 'INVITED' | 'ONBOARDED') => {
